...
1. ポンプの回転数 (RPM)
  ...
  「手動回転数モード」が有効になります。
  操作ピン: アナログ入力の A0ピン（RPM_ANALOG_IN_PIN）に接続された
  可変抵抗（ボリューム）を回すことで、回転数を調整できます。
  挙動: ピン42が開放（HIGH）の場合は固定値（NORMAL_MAX_RPM：2400〜2500rpm）
  で動作しますが、LOWの場合は可変抵抗の値を元に計算された回転数が目標値となります。
2. ポンプ運転判断の電流しきい値
  制御方法: ピン43（MANUAL_THRESHOLD_MODE_PIN）をGNDに接続（LOW）することで
  「電流しきい値可変モード」が有効になります。
  操作ピン: アナログ入力の A2ピン（THRESHOLD_ANALOG_IN_PIN）に接続された
  可変抵抗を操作することで、ポンプの運転継続を判断するための電流しきい値を調整できます。
  設定範囲: 可変抵抗の読み取り値（0〜1023）に応じて、しきい値を500〜950の範囲で変更可能です。
3. UVランプの接続本数 (モデル自動検出)
  制御方法: A3ピンからA6ピン（UV_DETECT_BIT0_PIN〜BIT3_PIN）の4ビットを
2進数として読み取ることで、使用するUVランプの本数を設定できます。
  可変範囲: プログラム上は15本まで実装可能ですが、現在は安全のため最大10本までに制限されています。
4. アワーメーターの動作モードと各種設定
  制御方法: A8ピンからA15ピンに接続されたDIPスイッチの状態を読み取ることで、
  動作仕様を切り替えられます。
  可変項目:
  停電復旧設定: SW1（A8ピン）により、電源復帰後に自動で運転を再開するか
  どうかを選択できます。
  アワーメーター動作条件: SW2〜SW4（A9〜A11ピン）の組み合わせにより、
